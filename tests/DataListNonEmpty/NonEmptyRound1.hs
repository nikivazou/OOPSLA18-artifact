...
tails = fromList . List.tails . toList
{-# INLINE tails #-}
...
group :: (Eq a) => NonEmpty a -> [NonEmpty a]
group = groupBy (==)
{-# INLINE group #-}
...
groupWith :: (Eq b) => (a -> b) -> NonEmpty a -> [NonEmpty a]
...
groupAllWith f = groupWith f . fromList . List.sortBy (compare `on` f) . toList 
{-# INLINE groupAllWith #-}
